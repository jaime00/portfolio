...
export default function ButtonDarkMode() {
  ...
  return (
    <span className="relative">
      ...
      <span
        className={`pointer-events-none absolute top-full left-1/2 mt-2 -translate-x-1/2 transition-all duration-fast [@media(hover:none)]:hidden ${hovered ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}
      >
        <span className="relative flex items-center rounded-lg border border-white/10 bg-gray-800 px-3 py-2 whitespace-nowrap shadow-[0_8px_24px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)]">
          ...
          <span className="absolute -top-[5px] left-1/2 h-2.5 w-2.5 -translate-x-1/2 rotate-45 border-t border-l border-white/10 bg-gray-800" />
        </span>
      </span>
    </span>
  )
